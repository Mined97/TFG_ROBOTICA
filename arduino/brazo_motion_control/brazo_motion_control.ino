...
int acceleration = 300;               // grados/seg^2 para limitar incrementos de velocidad

String rxBuffer;
unsigned long lastStepMs = 0;
float currentStepSpeed[NUM_CHANNELS] = {0, 0, 0, 0, 0, 0, 0};
...
void updateMotion() {
  ...
  for (byte i = 0; i < NUM_CHANNELS; i++) {
    ...
    // Movimiento progresivo con rampa sencilla de aceleracion.
    float dt = elapsed / 1000.0;
    float targetSpeed = speedForJoint(i);
    currentStepSpeed[i] += acceleration * dt;
    if (currentStepSpeed[i] > targetSpeed) currentStepSpeed[i] = targetSpeed;

    int maxStep = (int)(currentStepSpeed[i] * dt);
...
void handleFrame(String frame) {
  ...
  if (cmd == 'P') {
    ...
    applyHomeTarget();
    for (byte i = 0; i < NUM_CHANNELS; i++) currentStepSpeed[i] = 0;
    sendOk(F("H"));
  } else {
